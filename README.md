...
## Round Touch Display (FlightScnr UI)

This project runs on a **1080×1080 round touch LCD** on Raspberry Pi with a UI modeled after [FlightScnr](https://github.com/yashmulgaonkar/FlightScnr), using the [plane-tracker-rgb-pi](https://github.com/ajplotkin/plane-tracker-rgb-pi) backend (FR24, weather, web UI).

Set in `.env` / `/etc/plane-tracker.env`:

```bash
DISPLAY_WIDTH=1080
DISPLAY_HEIGHT=1080
DISPLAY_FULLSCREEN=True
# SDL_VIDEODRIVER=fbcon    # only if auto-detect fails; leave unset first
```

Install dependencies (`pygame` is included in `requirements.txt`), then start as usual:
